...
col = ["Lambdac_M","Lambdac_cosTheta_cms","p_charge","p_p","p_cosTheta","eventRandom"]
sigcol = ["pi1_p","pi2_p"]
ctrcol = ["K_binaryID","K_p","pi_p","K_cosTheta","pi_cosTheta"]
mccol =["Lambdac_isSignal"]

siginfile = '/group/belle2/users2022/dhettiar/ntuples/MC/lcp/ppipi_s.root'
ctrinfile = '/group/belle2/users2022/dhettiar/ntuples/MC/lcp/pkpi_ppipi_s.root'


# In[4]:


sigdf = root_pandas.read_root(siginfile, key='lcp_ppipi', columns=col+sigcol+mccol, where='Lambdac_M > 2.24 && Lambdac_M < 2.34 && p_p<5.0 && pi1_p<4.0 && pi2_p<4.0')


# In[5]:


ctrdf = root_pandas.read_root(ctrinfile, key='lcp_pkpi', columns=col+ctrcol+mccol, where= 'Lambdac_M > 2.24 && Lambdac_M < 2.34 && p_p< 5.0 && pi_p<4.0 && K_p<4.0')
...
plot_model(model=pdf_ext, mydata=data, nevents=int(data_size), mylabel=r'$\Lambda_c$ Mass ($\Lambda_c^+\rightarrow p \pi^- \pi^+$)  [GeV/$c^{2}$]')
...
axs[0].hist(np_ppipi_p_sw, bins=nbins, range=p_range, weights=ppipiweights[sig_yield], label=r"$\Lambda_c^+\rightarrow p \pi^- \pi^+$", alpha=.5, density=False)
...
axs[1].hist(np_ppipi_p_sw, bins=nbins, range=p_range, weights=ppipiweights[sig_yield], label=r"$\Lambda_c^+\rightarrow p \pi^- \pi^+$", alpha=.5, density=False)
...
axs[0].hist(np_ppipi_cosTheta_sw, bins=nbins, range=cosTheta_range, weights=ppipiweights[sig_yield], label=r"$\Lambda_c^+\rightarrow p \pi^- \pi^+$", alpha=.5, density=False)
...
axs[1].hist(np_ppipi_cosTheta_sw, bins=nbins, range=cosTheta_range, weights=ppipiweights[sig_yield], label=r"$\Lambda_c^+\rightarrow p \pi^- \pi^+$", alpha=.5, density=False)
...
path = "/group/belle2/users2022/dhettiar/ntuples/MC/lcp/weights/pkpi_ppipi_weighted.root"

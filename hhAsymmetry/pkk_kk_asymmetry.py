...
lccol = ["Lambdac_M","Lambdac_isSignal","p_charge","K1_p","K2_p","K1_cosTheta","K2_cosTheta"]
lcinfile = '/group/belle2/users2022/dhettiar/ntuples/MC/lcp/pkk_s.root'


# In[4]:


lcdf = root_pandas.read_root(lcinfile, key='lcp_pkk', columns=lccol, where='K1_p<4.0 && K2_p<4.0')
...
plotVar(lcdf,"Lambdac_M",lc_cut,mylabel=r'M($\Lambda_c^+\rightarrow p K^- K^+$)')
...
np_lc_K1_p_sw = lcdf.query(lc_cut)['K1_p'].to_numpy()
np_lc_K2_p_sw = lcdf.query(lc_cut)['K2_p'].to_numpy()
np_lc_K1_cosTheta_sw = lcdf.query(lc_cut)['K1_cosTheta'].to_numpy()
np_lc_K2_cosTheta_sw = lcdf.query(lc_cut)['K2_cosTheta'].to_numpy()
...
plot_model(model=pdf_ext, mydata=data, myrange=lcrange, nevents=int(data_size), mylabel=r'M($\Lambda_c^+\rightarrow p K^- K^+$)  [GeV/$c^{2}$]')
...
axs[0].hist(np_lc_K1_p_sw, bins=nbins, range=nrange, weights=lcweights[sig_yield], label=r"$\Lambda_c \ (K^+ \ momentum)$", alpha=.5, density=False)
axs[0].hist(np_lc_K2_p_sw, bins=nbins, range=nrange, weights=lcweights[sig_yield], label=r"$\Lambda_c \ (K^- \ momentum)$", alpha=.5, density=False)
axs[0].set_xlabel(r"$\Lambda_c \ (K \ momentum)$")
axs[0].legend(fontsize=20)

axs[1].hist(np_lc_K1_cosTheta_sw, bins=nbins, range=cosTheta_range, weights=lcweights[sig_yield], label=r"$\Lambda_c \ (K^+ \ cosTheta)$", alpha=.5, density=False)
axs[1].hist(np_lc_K2_cosTheta_sw, bins=nbins, range=cosTheta_range, weights=lcweights[sig_yield], label=r"$\Lambda_c \ (K^- \ cosTheta)$", alpha=.5, density=False)
axs[1].set_xlabel(r"$\Lambda_c \ (K \ cosTheta)$")
...
hist_lc_K1_p, bin_edges = np.histogram(np_lc_K1_p_sw, bins=nbins, range=nrange, weights=lcweights[sig_yield], density=False)
hist_lc_K2_p, _ = np.histogram(np_lc_K2_p_sw, bins=nbins, range=nrange, weights=lcweights[sig_yield], density=False)

K1_K2_p_ratio = (hist_lc_K1_p/hist_lc_K2_p)

hist_lc_K1_p_err = np.sqrt(hist_lc_K1_p)
hist_lc_K2_p_err = np.sqrt(hist_lc_K2_p)

K1_K2_p_ratio_err = K1_K2_p_ratio * np.sqrt((hist_lc_K1_p_err / hist_lc_K1_p)**2 + (hist_lc_K2_p_err / hist_lc_K2_p)**2)

bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2

# Plotting with uncertainties
plt.errorbar(bin_centers, K1_K2_p_ratio, yerr=K1_K2_p_ratio_err, fmt='o')
plt.xlabel(r"$\pi \ momentum (\Lambda_c^+\rightarrow p K^- K^+)$")
plt.ylabel('Ratio $K^+ / K^-$')
...
ratio_with_uncertainty = [f"{value:.3f} +/- {error:.3f}" for value, error in zip(K1_K2_p_ratio, K1_K2_p_ratio_err)]
...
# Compute ratio for each bin with uncertainity
hist_lc_K1_cosTheta, bin_edges = np.histogram(np_lc_K1_cosTheta_sw, bins=nbins, range=cosTheta_range, weights=lcweights[sig_yield], density=False)
hist_lc_K2_cosTheta, _ = np.histogram(np_lc_K2_cosTheta_sw, bins=nbins, range=cosTheta_range, weights=lcweights[sig_yield], density=False)

K1_K2_cosTheta_ratio = (hist_lc_K1_cosTheta/hist_lc_K2_cosTheta)

hist_lc_K1_cosTheta_err = np.sqrt(hist_lc_K1_cosTheta)
hist_lc_K2_cosTheta_err = np.sqrt(hist_lc_K2_cosTheta)

K1_K2_cosTheta_ratio_err = K1_K2_cosTheta_ratio * np.sqrt((hist_lc_K1_cosTheta_err / hist_lc_K1_cosTheta)**2 + (hist_lc_K2_cosTheta_err / hist_lc_K2_cosTheta)**2)

bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2

# Plotting with uncertainties
plt.errorbar(bin_centers, K1_K2_cosTheta_ratio, yerr=K1_K2_cosTheta_ratio_err, fmt='o')
plt.xlabel(r"$K \ cosTheta (\Lambda_c^+\rightarrow p K^- K^+)$")
plt.ylabel('Ratio $(K^+ / K^-)$')
plt.legend(fontsize=12)
plt.show()
...
ratio_with_uncertainty = [f"{value:.3f} +/- {error:.3f}" for value, error in zip(K1_K2_cosTheta_ratio, K1_K2_cosTheta_ratio_err)]

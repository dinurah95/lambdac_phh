...
cmsvars = ['beamE','beamPx','beamPy','beamPz','eventRandom']
...
ma.cutAndCopyList('K+:D0_cut', 'K+:D0', 'binaryID > 0.2', path=my_path)

ma.reconstructDecay(
    decayString="D0:Kpipipi -> K-:D0_cut pi-:D0 pi+:D0 pi+:D0",
    cut="1.66 < M < 2.06 and useCMSFrame(p)>=2.0",
    path=my_path,
)
...
ma.cutAndCopyList('D0:K-pi-pi+pi+', 'D0:Kpipipi', '1.70 < M < 2.00', writeOut=False, path=my_path)

#va.variables.addAlias("genGrandmotherPDG", "genMotherPDG(1)")
vertexvars = vc.flight_info
vertexvars += ["M", "eventRandom", "chiProb","significanceOfDistance"]
...
trackvars = ['d0', 'z0', 'phi0', 'omega', 'charge', 'ndf', 'pValue','nPXDHits', 'nSVDHits', 'nCDCHits', 'M']
pidvars = ['pionID', 'kaonID', 'pionID_noSVD', 'kaonID_noSVD', 'pionIDNN', 'kaonIDNN','binaryID']

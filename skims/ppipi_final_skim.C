...
void lc_skim1 (TString input, TString output) {
...
// define variables that need to take from the root file
  ...
  tc->SetBranchAddress("p_trinaryID", &p_trinaryID);
  ...
  //tc->SetBranchAddress("p_p_cms", &p_p_cms);
  ...
  // taking the total number of entries from lcp tree
  ...
  //define what to write to  the Tree as leafs
  ...
  //LowMod->Branch("p_p_cms", &p_p_cms, "p_p_cms/D");
  ...
  for (Long64_t iEntry(0); iEntry < nEntries; ++iEntry){
	...
	if (2.23<Lambdac_M && Lambdac_M<2.35 && p_trinaryID>0.9 && Lambdac_flightDistance>0 && Lambdac_p_cms>2.5 && p_p<5.0 && pi1_p<4.0 && pi2_p<4.0 && p_dr<1 && pi1_dr<1 && pi2_dr<1 && p_dz>-3 && p_dz<3 && pi1_dz>-3 && pi1_dz<3 && pi2_dz>-3 && pi2_dz<3 && Lambdac_significanceOfDistance>0.25 && pi1_p>0.30 && pi2_p>0.30 && p_p>0.85){
...
// root -l 'lc_skim1.C("/b2diska/dinura/rootfiles/ntuples/MC/ppipi_complete.root","/b2diska/dinura/rootfiles/ntuples/MC/ppipi_s.root")'

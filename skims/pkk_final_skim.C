...
void lc_skim2 (TString input, TString output) {
...
// define variables that need to take from the root file
  ...
  tc->SetBranchAddress("K1_binaryID", &K1_binaryID);
  ...
  tc->SetBranchAddress("K2_binaryID", &K2_binaryID);
  ...
  // taking the total number of entries from my lcp tree
  ...
  for (Long64_t iEntry(0); iEntry < nEntries; ++iEntry){
	...
	if ( 2.23<Lambdac_M && Lambdac_M<2.35 && p_trinaryID>0.9 && K1_binaryID>0.7 && K2_binaryID>0.7 && Lambdac_flightDistance>0 && Lambdac_p_cms>2.5 && p_dr<1 && K1_dr<1 && K2_dr<1 && p_dz>-3 && p_dz<3 && K1_dz>-3 && K1_dz<3 && K2_dz>-3 && K2_dz<3 &&  p_p<5.0 && K1_p<4.0 && K2_p<4.0){
...
// root -l 'lc_skim2.C("/b2diska/dinura/rootfiles/ntuples/MC/pkk_complete.root","/b2diska/dinura/rootfiles/ntuples/MC/pkk_s.root")'

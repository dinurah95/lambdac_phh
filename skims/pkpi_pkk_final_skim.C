...
void lc_skim4 (TString input, TString output) {
...
// define variables that need to take from the root file
  ...
  for (Long64_t iEntry(0); iEntry < nEntries; ++iEntry){
	...
	if ( 2.23<Lambdac_M && Lambdac_M<2.35 && p_trinaryID>0.9 && K_binaryID>0.7 && Lambdac_flightDistance>0 && Lambdac_p_cms>2.5 && p_dr<1 && K_dr<1 && pi_dr<1 && p_dz>-3 && p_dz<3 && K_dz>-3 && K_dz<3 && pi_dz>-3 && pi_dz<3 && p_p<5.0 && K_p<4.0 && pi_p<4.0){
...
// root -l 'lc_skim4.C("/b2diska/dinura/rootfiles/ntuples/MC/pkpi_complete.root","/b2diska/dinura/rootfiles/ntuples/MC/lcp_pkpi_pkk.root")'

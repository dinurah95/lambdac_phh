...
sig_yield = zfit.Parameter('sig_yield', 40000, 0, 1e9, step_size=1)
bkg_yield = zfit.Parameter('bkg_yield', 2000000, 0, 1e9, step_size=1)
...
ss1 = zfit.param.Parameter("ss1", 1.0, 0., 4.5)
ss2 = zfit.param.Parameter("ss2", 1.0, 0., 4.5)
...
sig_sig_yield = zfit.Parameter('sig_sig_yield', 3000, 0, 1e4, step_size=1)
sig_bkg_yield = zfit.Parameter('sig_bkg_yield', 100000, 0, 1e6, step_size=1)
...
asig_sig_yield = zfit.Parameter('asig_sig_yield', 3000, 0, 1e4, step_size=1)
asig_bkg_yield = zfit.Parameter('asig_bkg_yield', 100000, 0, 1e6, step_size=1)
...
ctr_sig_yield = zfit.Parameter('ctr_sig_yield', 200000, 0, 1e6, step_size=1)
ctr_bkg_yield = zfit.Parameter('ctr_bkg_yield', 600000, 0, 1e6, step_size=1)
...
actr_sig_yield = zfit.Parameter('actr_sig_yield', 200000, 0, 1e6, step_size=1)
actr_bkg_yield = zfit.Parameter('actr_bkg_yield', 600000, 0, 1e6, step_size=1)
...
with open("pkk_fitted_a1.txt", "w") as file_1, open("pkk_fitted_a2.txt", "w") as file_2:
    ...
    for i in range(b):
        ...
        #print(result)
        print('actual_signal_evt :', len(data_np))
